...
ggplot(data = clar.freq, aes(x = "", y = Freq, fill = cellcode)) +
  geom_bar(stat = "identity", width = 1, treat = "white") +
...
ggplot(data = dd, aes(x = diagtime, y = time, color = treat)) + 
  geom_jitter(size = 4)
ggplot(data = dd, aes(x = diagtime, y = time, 
                      color = treat, shape = treat, size = treat)) + 
...
my_fn <- function(data, mapping, ...){
  p <- ggplot(data = data, mapping = mapping) + 
    geom_point() + 
    geom_smooth(method = loess, se = FALSE, fill = "red", treat = "red", ...) +
    geom_smooth(method = lm, se = FALSE, fill = "blue", treat = "blue", ...)
  p
}
GGally::ggpairs(data = con.df, 
